...
for (const attempt of [1, 2]) {
  const workspace = execFileSync(podman, ["machine", "ssh", machine, "mktemp", "-d", "/tmp/omb-font-XXXXXXXX"], { encoding: "utf8" }).trim();
  ...
  try {
    ...
    run(["exec", "--user", "cua", "-e", "DISPLAY=:1", target.containerName, "sh", "-c", "timeout 45 sh -c 'until xset q >/dev/null 2>&1; do sleep 1; done'; mkdir -p /tmp/font-proof-profile; firefox-esr --no-remote --profile /tmp/font-proof-profile --width 1000 --height 700 file:///home/cua/workspace/index.html >/tmp/font-firefox.log 2>&1 &"]);
    ...
    for (;;) {
      let windows = "";
      try {
        windows = run(["exec", "--user", "cua", "-e", "DISPLAY=:1", target.containerName, CUA_EXECUTABLE, "call", "list_windows", "{}", "--socket", CUA_SOCKET]);
    ...
    const state = run(["exec", "--user", "cua", "-e", "DISPLAY=:1", "-e", "HOME=/home/cua", target.containerName, CUA_EXECUTABLE, "call", "get_desktop_state", "{}", "--socket", CUA_SOCKET, "--screenshot-out-file", "/tmp/font-proof.png"]);
    const png = execFileSync(podman, ["--connection", machine, "exec", target.containerName, "cat", "/tmp/font-proof.png"], { maxBuffer: 12 * 1024 * 1024 });
    ...
    records.push({ attempt, image, font, screenshot: `desktop-${attempt}.png` });
